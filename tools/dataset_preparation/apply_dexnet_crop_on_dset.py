...
from gqcnn.utils import centre_rotate_crop_resize

"""
Applies random cropping to a dataset. Can be used on Validation or Testing dataset to ensure comparability when
multiple models are to be tested on a dataset.
"""
...
# Paths to dataset
dset = 'overhead-A-xyzq_Testing'
data_dir = DATA_PATH + dset + '/tensors/'
validation_dir = DATA_PATH + dset + '_dexnet/'
...
for tensor in tqdm(tensors_in_dir):
    ...
    for cnt in range(0, len(data['image_labels'])):
        ...
        cropped_im = centre_rotate_crop_resize(im,
                                               grasp[0],
                                               grasp[1],
                                               grasp[3])
