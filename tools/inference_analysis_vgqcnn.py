...
class GQCNN2:
    ...
    def build(self):
        ...
        return tf.keras.Model(inputs=[im.input, pose.input], outputs=merge)
...
def _preprocess(image, poses):
    ...
    preprocessed_images = np.zeros((len(poses), 32, 32, 1))

    for cnt, pose in enumerate(poses):
        # Centre images
         centred_im = image[350-pose[0].astype(int):550-pose[0].astype(int),
                            350-pose[1].astype(int):550-pose[1].astype(int),
                            0]
         im = Image.fromarray(centred_im).crop((52, 52, 148, 148)).resize((32, 32),
                                                                          resample=Image.BILINEAR)
         preprocessed_images[cnt] = np.asarray(im).reshape(32, 32, 1)

    depth_im, depth_data = normalise_values(preprocessed_images, poses[:, 2:7])

    return depth_im, depth_data

@stats_n_times(ITERATIONS)
def _predict(model, image, poses):
    model([image, poses])
...
def inference_analysis(model_dir, data_dir):
    model = GQCNN2(model_dir).build()

    estimate_flops(model_dir)

    model.compile(run_eagerly=False, loss=tf.losses.mean_squared_error)

    for bs in BATCH_SIZES:
        if '/home/' in data_dir:
            output_dir = '/home/anna/Grasping/results/inference/{}_orig_cpu.txt'.format(bs)
        else:
            output_dir = '/results/inference/{}_orig_gpu.txt'.format(bs)
        ...
        predict_std = []
        for i in range(RUNS):
            mean, std = _predict(model, depth_im, depth_data)
